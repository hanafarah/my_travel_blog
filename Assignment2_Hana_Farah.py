...
def question_1_return_active_users():
    ...
    """
    return User.objects.filter(active_users=True) 
...
def question_2_return_regular_users():
    ...
    """
    return User.objects.filter(not_staff=True, not_superuser=True)
...
def question_5_return_all_post_comments(post):
    ...
    of last created.
    """
    return Comments.objects.all().filter(post=post).order_by('created_on')
...
def question_6_return_the_post_with_the_most_comments():
    ...
    """
    Post.objects.annotate(num_comment=Count('comments')).order_by('num_comments')
...
def question_7_create_a_comment(post):
    ...
    """
    return Comment.objects.create(post=post, comment = 'Good post')
...
def question_8_set_approved_to_false(comment):
    ...
    """
    return Comment.objects.update(set_approved =False)
...
def question_9_delete_post_and_all_related_comments(post):
    ...
    """
    return Post.objects.all(post=post).delete()
